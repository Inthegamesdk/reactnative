...
import { UIManager, Platform, findNodeHandle, StyleSheet, BackHandler, SafeAreaView, useWindowDimensions } from 'react-native';
...
const _callNativeFunction = (overlay, eventName, methodArgs) => {
  if (methodArgs != null && Array.isArray(methodArgs)) {
    UIManager.dispatchViewManagerCommand(findNodeHandle(overlay), eventName, methodArgs);
...
const ITGVideoOverlay = props => {
  ...
  const _updateOverlayPlayingState = () => {
    let methodName = isVideoPlaying ? NativeFunctions.videoPlaying : NativeFunctions.videoPaused;
    if (Platform.OS === 'ios') {
  ...
  useEffect(() => {
    if (Platform.OS === 'android') {
      _callNativeFunction($bridge.current, NativeFunctions.setup, [findNodeHandle($bridge.current), findNodeHandle($video.current)]);
      KeyEvent.onKeyDownListener(keyEvent => {
        _callNativeFunction($bridge.current, 'receivedKeyEvent', keyEvent.keyCode);
      });
    } else if (Platform.OS === 'ios') {
      _callNativeFunction($bridge.current, NativeFunctions.setup, [findNodeHandle($bridge.current)]);
    }
  ...
  }), /*#__PURE__*/React.createElement(ITGOverlayView, {
